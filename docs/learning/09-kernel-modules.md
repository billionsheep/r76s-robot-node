...
**状态：首轮 34799529908 已完成编译，但模块版本检查失败，正在修复后复验。以下完整模块包尚未通过验收。**

首轮日志显示树内模块与 r8125 均完成编译和安装，检查在 `seqiv.ko` 的空 vermagic 处停止。检查脚本原先调用 `modinfo -F vermagic 文件路径`，仍使用运行器的默认内核查询上下文。旧版 kmod 源码中，即使传入文件路径，读取信息时也会按名称查询该上下文的内置模块。

修复将全部模块信息查询改为 `modinfo -b "$STAGE" -k "$RELEASE" ...`：`-b` 指向本次安装的目标目录，`-k` 指定本次目标内核版本。再次构建会保留 `modinfo-context.txt`，对照原查询、目标查询与 ELF 文件原始字段，验证是否确为宿主同名模块干扰；不会通过跳过模块或放宽版本检查来绕过失败。参考 [kmod v29 源码](https://github.com/kmod-project/kmod/blob/v29/libkmod/libkmod-module.c)。
