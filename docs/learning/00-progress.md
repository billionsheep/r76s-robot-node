...
当前学习：[给内核加版本标识并对照实际结果](06-kernel-version-label.md)。手动运行 `34666871155` 成功，生成 `6.1.141-r76s-study1`，完整配置只差版本后缀一行，产物下载校验通过。接着沿日志解释执行过程，再推进 U-Boot/启动固件链。北大编译课程留在另一份笔记，不作为主线前置；原始构建的文件关系见 [导读](05-read-the-real-build.md)。
