...
当前学习：[U-Boot 的目录、配置、命令与产物](07-uboot-source-to-output.md)。已只读核对厂商 RK3576 专用脚本、`nanopi_m5_defconfig`、固件清单和复制规则；U-Boot 工作流与编译尚未实施。上一节手动运行 `34666871155` 成功，生成 `6.1.141-r76s-study1`，完整配置只差版本后缀一行，产物下载校验通过，见 [版本实验](06-kernel-version-label.md)。北大编译课程留在另一份笔记，不作为主线前置。
