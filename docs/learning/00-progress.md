...
当前学习：[把 U-Boot 接入项目，逐文件理解工作流](08-uboot-workflow.md)。已准备仅手动触发的工作流、构建脚本和固定输入；语法、误调用保护与六项固件输入预检查通过，首轮云端构建与产物验证待用户启动后进行。厂商来源见 [源码导读](07-uboot-source-to-output.md)。上一节手动运行 `34666871155` 已生成 `6.1.141-r76s-study1`，完整配置只差版本后缀一行，产物下载校验通过，见 [版本实验](06-kernel-version-label.md)。北大编译课程留在另一份笔记，不作为主线前置。
